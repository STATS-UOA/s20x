...
vignette: >
  %\VignetteIndexEntry{Case Study 9.2: Thyroid drug experiment}
...
```{r RC-CS92-001, out.width = "70%", fig.align = 'center',eval=FALSE}
thyroid.df = read.table("thyroid-1.txt", header = TRUE)
head(thyroid.df)
str(thyroid.df)
```
...
```{r RC-CS92-005, out.width = "75%", fig.align = 'center', eval=requireNamespace("ggplot2", quietly = TRUE)}
b = coef(thyroid.fit2)
...
```{r RC-CS92-008, out.width = "85%", fig.align = 'center', fig.width = 8, fig.height = 6}
set.seed(234)
library(bootstrap)
...
plot(density(b[1, ]), main = expression(beta[0])); abline(v = 0)
plot(density(b[2, ]), main = expression(beta[1])); abline(v = 0)
plot(density(b[3, ]), main = expression(beta[2])); abline(v = 0)
```
